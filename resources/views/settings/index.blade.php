...
@section('page_title', __('voyager::voyager.generic.viewing').' '.__('voyager::voyager.generic.settings'))
...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-settings"></i> {{ __('voyager::voyager.generic.settings') }}
    </h1>
@stop
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="alert alert-info">
            <strong>{{ __('voyager::voyager.generic.how_to_use') }}:</strong>
            <p>{{ __('voyager::voyager.settings.usage_help') }} <code>setting('group.key')</code></p>
        </div>
        @endif
    </div>
    ...
    <div class="page-content settings container-fluid">
        <form action="{{ route('voyager.settings.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" class="btn btn-primary pull-right">{{ __('voyager::voyager.settings.save') }}</button>
        </form>
        ...
        <div class="panel" style="margin-top:10px;">
            <div class="panel-heading new-setting">
                <hr>
                <h3 class="panel-title"><i class="voyager-plus"></i> {{ __('voyager::voyager.settings.new') }}</h3>
            </div>
            <div class="panel-body">
                <form action="{{ route('voyager.settings.store') }}" method="POST">
                    ...
                    <div class="col-md-3">
                        <label for="key">{{ __('voyager::voyager.generic.key') }}</label>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="type" class="form-control" required="required">
                            ...
                            <option value="text_area">{{ __('voyager::voyager.form.type_textarea') }}</option>
                            <option value="rich_text_box">{{ __('voyager::voyager.form.type_richtextbox') }}</option>
                            <option value="code_editor">{{ __('voyager::voyager.form.type_codeeditor') }}</option>
                            <option value="checkbox">{{ __('voyager::voyager.form.type_checkbox') }}</option>
                            <option value="radio_btn">{{ __('voyager::voyager.form.type_radiobutton') }}</option>
                            <option value="select_dropdown">{{ __('voyager::voyager.form.type_selectdropdown') }}</option>
                            <option value="file">{{ __('voyager::voyager.form.type_file') }}</option>
                            <option value="image">{{ __('voyager::voyager.form.type_image') }}</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label for="group">{{ __('voyager::voyager.settings.group') }}</label>
                    ...
                    <div class="col-md-12">
                        <a id="toggle_options"><i class="voyager-double-down"></i> {{ mb_strtoupper(__('voyager::voyager.generic.options')) }}</a>
                        <div class="new-settings-options">
                            <label for="options">{{ __('voyager::voyager.generic.options') }}
                                <small>{{ __('voyager::voyager.settings.help_option') }}</small>
                            </label>
                            ...
                            <div id="valid_options" class="alert-success alert" style="display:none">{{ __('voyager::voyager.json.valid') }}</div>
                            <div id="invalid_options" class="alert-danger alert" style="display:none">{{ __('voyager::voyager.json.invalid') }}</div>
                        </div>
                    </div>
                    <div style="clear:both"></div>
                    <button type="submit" class="btn btn-primary pull-right new-setting-btn">
                        <i class="voyager-plus"></i> {{ __('voyager::voyager.settings.add_new') }}
                    </button>
                    <div style="clear:both"></div>
                </form>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('voyager::voyager.generic.close') }}">
                        <span aria-hidden="true">&times;</span>
                    </button>
                    <h4 class="modal-title">
                        <i class="voyager-trash"></i> {!! __('voyager::voyager.settings.delete_question', ['setting' => '<span id="delete_setting_title"></span>']) !!}
                    </h4>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default pull-right" data-dismiss="modal">{{ __('voyager::voyager.generic.cancel') }}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('javascript')
    ...
    $(".group_select_new").select2({
        tags: true,
        width: 'resolve',
        placeholder: '{{ __("voyager::voyager.generic.select_group") }}'
    });
